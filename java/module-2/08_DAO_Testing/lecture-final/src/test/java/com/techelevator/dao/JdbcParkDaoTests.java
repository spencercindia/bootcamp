...
public class JdbcParkDaoTests extends BaseDaoTests {
    ...
    public void setup() {
        ...
        testPark = new Park(0, "Test Park", LocalDate.now(), 99, true);
    }
    ...
    public void deleted_park_cant_be_retrieved() {
        ...
        Park parkToDelete = sut.getPark(1);
        Assert.assertNull(parkToDelete);
    ...
    public void park_added_to_state_is_in_list_of_parks_by_state() {
        Park parkToAdd = sut.createPark(testPark);
        long newId = parkToAdd.getParkId();
        sut.addParkToState(newId, "CC");
        List<Park> parks = sut.getParksByState("CC");
        Assert.assertEquals(2, parks.size());
        assertParksMatch(parkToAdd, parks.get(1));
    }
